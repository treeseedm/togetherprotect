...
public class TakePhotoActivity extends Activity implements OnClickListener {
    ...
    public int mFromPage = 0;// 0 代表选择头像 1 代表发表评价
    ...
    private void initParams() {
        mFromPage = getIntent().getIntExtra("frompage", 0);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_TAKE_PHOTO && mFromPage == 1) {// 来自评价晒单页面， 从照相机返回，直接回到评价晒单页面
            data = new Intent();
        ...
        if (requestCode == SELECT_PHOTO && mFromPage == 1) {// 来自评价晒单页面， 从相册返回，直接回到评价晒单页面
            setResult(Activity.RESULT_OK, data);
